...
Evidence reuse applies to checks run by the owner as well as workers, including documentation queries, schema inspection and temporary database setup. Record the checked revision or surface, relevant dependencies/environment, exact command and result so final verification can assess freshness. Rebuild an environment or repeat migrations only when relevant inputs changed, evidence is incomplete or doubtful, or a distinct accepted path requires fresh-state proof. A lifecycle checkpoint, owner rotation or entry into final verification alone does not invalidate an otherwise current result. Still inspect the complete final diff and account for every accepted outcome; an exploratory check does not prove a later edited runbook unless the exact final operation was exercised.
