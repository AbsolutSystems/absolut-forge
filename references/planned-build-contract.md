...
Treat the active orchestrator context as disposable. At every completed-task boundary, the committed Brief, plan, source, tests, and Git history must be sufficient for a fresh host-mapped orchestrator to continue without the previous conversation.
...
Use the active host's value-triggered fresh-owner continuation when context pressure, a changed decision frame or required independence warrants it. A checkpoint or entry into final integration alone does not force rotation. If the host has no such primitive, compact to durable current facts and continue only when the current owner remains eligible and safe continuation does not require isolation. Otherwise stop at the clean boundary under the host's unavailable-rotation rule; compaction does not provide independence. Use no fixed task count. Normal resume reads runtime, Brief status and accepted intent/amendments, plan header and Active Frontier, current task, and relevant code/tests. Load completion evidence only for direct dependencies whose necessary facts are absent or uncertain. Do not preload all completed bodies, plan-change history, full schemas/doctrine, unrelated host mappings or the base-to-HEAD diff. Use current code as implementation truth; individual checkpoint diffs need a concrete regression, ownership or history question.
