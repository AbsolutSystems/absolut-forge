...
Do not let one unresolved high-tier decision absorb otherwise delegable implementation. At compilation, separate that decision from a subsequent coherent implementation task when resolving and recording it produces settled contracts, bounded ownership and a meaningful gate. Reclassify the residual task on its own execution risk. This is a real dependency/risk boundary, not permission to fragment by file or to encode most of the patch in a worker capsule. Keep the implementation high when executing it still crosses the architecture, migration, security/data, concurrency or state boundary, and state that concrete reason in the task.
...
Use the lowest capability tier that can safely execute the coherent task under Task design. Low covers mechanical/local work with explicit contracts and fast gates; standard covers bounded multi-file behavior with settled contracts, implementation/wiring and focused tests; high covers shared architecture, migration, security/data boundaries, concurrency/state complexity or material ambiguity. Classify the unresolved decision and residual implementation separately: once the orchestrator records a high-tier decision at a real dependency/risk boundary, reclassify the bounded implementation that follows instead of inheriting `high` from the enclosing outcome. Do not fragment standard work merely to lower its tier, or downgrade risk because a worker has more reasoning effort. Evidence of underestimated complexity requires a PC revision escalating the pending task or splitting it at a real boundary. A worker fixes local failures and repeats its fast gate inside the current task; it returns only at the Task Capsule boundary. Classify failures discovered later by orchestrator validation or final verification before editing: local constructor, fixture, assertion, type, formatting and settled API-usage corrections retain their low/standard tier and return to a fresh bounded worker; failures exposing unresolved intent, architecture, data integrity, security, migration, concurrency or cross-task inconsistency are high-tier corrections owned by the orchestrator. Legacy delegated restrictions remain in force. Model names, reasoning profiles and unavailable-dispatch mechanics belong in the active host mapping.
