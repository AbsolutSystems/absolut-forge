...
Autonomous execution may delegate one bounded outcome under `model-routing.md#autonomous-outcome-routing`; that handoff alone is not a reason to select planned. Delegation justifies a graph only when coordinating multiple tasks provides the benefit above. Inline versus worker execution does not change recorded strategy or methodology and adds no artifact requirement.
