...
Classify unresolved judgment separately from the implementation that follows it. The presence of one high-tier concern does not make every source and test edit in the enclosing outcome high. Before assigning implementation to the owner, determine whether the owner can resolve and record a genuine architecture, migration, security/data, concurrency or state boundary, leaving a coherent slice with settled contracts, bounded ownership and a meaningful fast gate. If so, split at that decision boundary, reclassify the remaining implementation, and delegate it when low/standard. Do not manufacture a decision-only split, prewrite most of the patch in the handoff, or downgrade work whose execution itself crosses the high-risk boundary. If residual implementation remains high, record the concrete unresolved judgment or execution risk that prevents delegation.
...
Choose execution separately from Build strategy, using the low/standard/high criteria and decision-boundary split above. No need for a task graph does not imply low risk. Delegate a coherent low/standard outcome with settled contracts, bounded write ownership and a meaningful fast gate to one fresh worker by default. Keep it inline only when the owner can complete it with fewer owner interactions than preparing and validating the handoff, and record that reason in the outcome evidence. A mixed outcome is not wholly high merely because the owner must first settle one high-tier decision: resolve and record that decision, then reclassify and delegate the residual implementation when safely bounded. If residual implementation remains high, record the concrete unresolved judgment or execution risk that prevents delegation. High-tier work stays with the owner; greater worker reasoning effort does not lower its risk.
