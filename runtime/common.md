...
- Completed checkpoint state is durable; conversation state is disposable. Summarize useful facts and discard raw worker dialogue. At a substantial checkpoint and before final verification, use the active host's fresh-owner continuation when it provides one; otherwise compact to durable current facts and continue. Never claim a host actually isolated or evicted context unless it did.
