...
On a repeated pass, read the existing `review.md` before dispatch only far enough to build a compact continuity registry: every finding ID and severity, its root issue, resolution, smallest correction, `Correction attempts`, and the last reviewed revision. Pass that registry with the normal starting package. Prior findings are navigation evidence, not authority: independently verify them against current intent, diff and tests, and never inherit a prior reviewer's conclusion. Reuse the ID when the same root issue persists, regresses or was incompletely corrected; allocate a new ID only for a distinct root issue. A changed line, symptom or affected file does not by itself create a new root issue.
...
Use exactly one fresh high-capability generic read-only reviewer when the host provides isolation, preferably another model family when practical. Provide the same bounded starting package; builder dialogue is not reviewer evidence. If isolation is unavailable, an inline pass is valid only in a high-capability context and must be labeled `advisory (not fully isolated)`; otherwise stop before writing Review. A different role name alone does not establish independence.
...
Write only review.md and Brief lifecycle status. Never change execution artifacts, Build Evidence, source or tests. Use stable finding IDs with evidence, impact, smallest correction, `Correction attempts` and resolution. A new finding starts at zero. On a later pass, increment only when repository evidence shows that Build attempted a correction for the same root issue and the issue remains; do not count Review reruns without a correction. BLOCKING requires correction or missing delivery proof before Ship; FOLLOW-UP requires no correction to this feature before Ship. Never downgrade failed gates to FOLLOW-UP.

With blockers, set Brief Building and return to `build`, which resumes the recorded strategy without selecting again: autonomous stays autonomous; planned standard/missing methodology stays standard; planned delegated retains legacy restrictions. Legacy tdd has no current builder: require a compatible older release or explicit clean Ready restart. The Build owner classifies each blocker, settles any decision it exposes and compiles the correction; every production-code or test edit returns to a fresh worker. Planned corrections become Build-owned PC tasks, but Review never reopens the plan itself or dispatches an executor. After two evidenced failed correction attempts for the same root issue, or on material scope expansion, escalate to the human instead of issuing another ordinary Build loop.
