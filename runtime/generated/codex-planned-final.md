...
- `references/codex-tools.md` sha256 `680cbd9e64968c29a17af4a73a3d13e1348cd9cd70aadfe2e8926ac8b69566bc`; rule `codex-host`
...
The Build skill's routing is explicit authorization for the nested owner to use Codex's native `spawn_agent` primitive for required advice and workers. Once the bounded package is ready, invoke `spawn_agent` directly with the exact model, reasoning effort and `fork_turns="none"`; do not call `list_agents` first and do not look for dispatch inside a shell or tool-wrapper namespace. `list_agents` reports existing agents only: a result containing only `/root` means that no child is running. Do not query or interpret the active-agent list as a capability check. Being a nested owner does not prove that `spawn_agent` is absent. Treat a required profile as unavailable only when the primitive is absent from the actual tool registry or the direct attempted dispatch with the required model and reasoning effort is refused or errors. Report the concrete missing primitive or dispatch error; owner implementation and silent profile substitution are unavailable.
