...
For planned Build at a clean completed-task boundary, explain that the committed plan and Git state already support direct resume through `build`; do not create redundant save context unless the human still requests it. Save is useful for a mid-task or otherwise unresolved stop, including an unfinished delegated-worker correction.
