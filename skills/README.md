...
On Codex, the invoking context launches a fresh GPT-5.6 Sol `medium` Build owner before repository inspection. Clean implementation/test checkpoints return a compact `ROTATE` envelope so the launcher can replace it with a fresh sibling owner; the launcher never reads or judges Build state. Sol compiles decisions and supervises; fresh Luna workers own every low, standard and high production-code or test edit and correction. See [`../references/codex-tools.md`](../references/codex-tools.md).
