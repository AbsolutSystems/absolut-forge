...
- a current new or materially changed guard lacks credible recorded targeted mutation proof.

A properly recorded `not available — {reason and closest whole-feature check}` value is not missing evidence; judge whether its reason and closest check are credible. Apply the explicit legacy mutation-evidence compatibility rule without rewriting old evidence. Never downgrade a delivery-gate defect to `FOLLOW-UP` because the missing information might be reconstructed from logs, another entry, test inspection, or a future Feature Record.
...
Handoff means report the eligible next skill and canonical artifact paths. Do not invoke `ship` unless the human explicitly invoked it or expressly authorized this request to continue through Ship.
