...
Keep the review independent: start from accepted intent, final Build Evidence, complete implementation diff, and changed/new tests. On a repeated pass, also project the bounded finding continuity registry from the existing Review report so stable IDs and correction-attempt escalation survive fresh context; previous conclusions remain non-authoritative. Assume Build already ran the required tests successfully when its final evidence is current and complete; do not rerun tests by default. Follow the runtime's targeted-rerun rule for concrete doubts. Do not preload execution history. For recorded delegated methodology, load its legacy contract and only the targeted ownership evidence the runtime requires. Write only the Brief lifecycle status and `review.md`; never repair implementation or Build evidence.
