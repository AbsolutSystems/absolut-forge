...
If valid, return exactly one next handoff matching the recorded strategy: `build` for autonomous or `build-planned` for planned. In other words, a planned save always resumes with `build-planned`. Never switch strategy, overwrite source, restore files, commit, stash or change branches.
