...
Both strategies share one positive, risk-based Test Charter. For each changed behavior, tests cover every applicable primary, failure/boundary, state/data, seam-contract, and regression obligation, with extra attention to security, persistence, compatibility, concurrency, and migrations. Test count follows distinct risks rather than task count. Intermediate checkpoints use narrow green unit targets, and workers report the exact tests they add or change. Final verification reuses current worker evidence, reruns only stale or doubtful affected targets, adds focused regressions and targeted integration/e2e checks for changed boundaries, and exercises the accepted primary path. A full suite is reserved for cross-cutting or otherwise unselectable risk; required PR CI may own broad unrelated regression coverage. Tests must establish repository-owned observable behavior rather than mock setup, framework/library behavior, or incidental implementation details; Review judges that semantic value from the tests and diff without deliberately breaking production code. Existing assertions are never weakened to reach green.
