...
reviewer extracts exactly `base_commit..HEAD` from the committed feature branch.
The primary context owns `review.md`, lifecycle transitions, and finding
normalization; Build exclusively owns implementation fixes and their
verification. Uncommitted source changes block Review; only the active
`review.md` is permitted as an uncommitted workflow artifact.
...
- **Pre-generated diff package:** rejected because it can be stale; the reviewer
  must inspect the committed branch range from `base_commit`.
...
- The primary context needs exact committed-range extraction and a clean-source
  worktree check.
